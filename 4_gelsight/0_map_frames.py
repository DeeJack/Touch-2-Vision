...
# Load the timestamps
time1 = np.load(
    os.path.join(VIDEO_FOLDER, "time1.npy")
)  # Timestamps for frames in "video.mp4"
time2 = np.load(
    os.path.join(VIDEO_FOLDER, "time2.npy")
)  # Timestamps for frames in "gelsight.mp4"
...
inpainted_frames = sorted(
    inpainted_frames, key=lambda x: int(x.stem.split("_")[1].replace(".jpg", ""))
)
gelsight_frames = sorted(
    gelsight_frames, key=lambda x: int(x.stem.split("_")[1].replace(".jpg", ""))
)

# Extract frame indices from the inpainted frame filenames (format 'frame_{count:0000}.jpg')
inpainted_indices = [int(frame.stem.split("_")[1]) for frame in inpainted_frames]
...
for idx in inpainted_indices:
    ...
    frame_mapping[inpainted_frames_folder / f"frame_{idx:04d}.jpg"] = gelsight_frames[
        closest_index
    ]
